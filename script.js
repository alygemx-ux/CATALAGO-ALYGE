...
// ✅ Normalizar datos para evitar errores con espacios o mayúsculas
function normalizeItem(raw) {
  ...
  return {
    ...
    Imagen_URL_2: norm["Imagen_URL_2"] ?? "", // ✅ Segunda imagen
...
async function init() {
  try {
    ...
    document.getElementById("productos").innerHTML =
      "<p style='text-align:center;color:#ccc;padding:20px;'>Error al cargar los productos.</p>";
  }
}
...
function mostrarCategorias() {
  ...
  cont.innerHTML =
    `<span class="categoria" onclick="mostrarTodos()">Todos</span>` +
    categorias
      .map(
        c =>
          `<span class="categoria" onclick="filtrar('${escapeHtml(c)}')">${escapeHtml(c)}</span>`
      )
      .join("");
}
...
// ✅ Mostrar productos con transición de imágenes
function mostrarProductos(lista) {
  const cont = document.getElementById("productos");
  if (!lista || lista.length === 0) {
    cont.innerHTML =
      `<p style="text-align:center;color:#ccc;padding:30px;">No se encontraron productos.</p>`;
    return;
  }
    ...
    .map(
      ...
      <div class="card">
        <div class="img-container">
          <img class="img1" src="${sanitizeUrl(p.Imagen_URL)}" alt="${escapeHtml(p.Nombre)}"
            onerror="this.src='https://via.placeholder.com/400x300?text=Sin+imagen'"/>
          ${
            p.Imagen_URL_2
              ? `<img class="img2" src="${sanitizeUrl(p.Imagen_URL_2)}" alt="${escapeHtml(p.Nombre)}"
        ...
        <div class="info">
          ...
          <strong class="precio">$${escapeHtml(p.Precio)}</strong>
...
function sanitizeUrl(u) {
  ...
  if (!/^https?:\/\//i.test(s)) return s; // ✅ permite URLs relativas como "images/..."
  return s;
}
